...
@Service
@Slf4j
public class CustomSubscriberScanner extends AbstractChannelScanner<Subscriber> implements ChannelsScanner, EmbeddedValueResolverAware {
    ...
    @Override
    protected Class<Subscriber> getListenerAnnotationClass() {
        return Subscriber.class;
    }

    @Override
    protected String getChannelName(Subscriber annotation) {
    ...
    private String getChannelNameFromQueues(Subscriber annotation) {
    ...
    private String getChannelNameFromBindings(Subscriber annotation) {
        return Arrays.stream(annotation.value())
                .map(resolver::resolveStringValue)
    ...
    @Override
    protected Map<String, ? extends ChannelBinding> buildChannelBinding(Subscriber annotation) {

        return Map.of("sqs", new SQSChannelBinding());
    }

    private String getExchangeName(Subscriber annotation) {
    ...
    @Override
    protected Map<String, ? extends OperationBinding> buildOperationBinding(Subscriber annotation) {

        return Map.of("sqs", new SQSOperationBinding());
    }

    /*private List<String> getRoutingKeys(Subscriber
    annotation) {
